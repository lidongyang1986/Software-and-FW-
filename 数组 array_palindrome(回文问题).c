	...
	{ 
		...
		{ 
			...
			if (L == 2) 
				...
				P[i][j] = (str[i] == str[j]) && P[i+1][j-1];                //检查子串是否对称：首尾 和 去掉首尾的 + 中间对称？
			...
			{ 
				...
				C[i][j] = INT_MAX; 
				for (k=i; k<=j-1; k++) 
					C[i][j] = min (C[i][j], C[i][k] + C[k+1][j] + 1);     //迭代算法：minPalPartion(str, i, j) = Min { minPalPartion(str, i, k) + 1 + minPalPartion(str, k+1, j) } 
